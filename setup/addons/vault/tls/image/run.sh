...
master() {
  ...
  if [ ! -f /root/vault_key.txt ]; then

    export VAULT_SKIP_VERIFY=true
    
    export VAULT_ADDR="https://${VAULT_IP}:${VAULT_HTTPS_PORT}"

    vault init -address=${VAULT_ADDR} > /root/vault_key.txt

    export VAULT_TOKEN=`grep 'Initial Root Token:' $LOG_PATH | awk '{print $NF}'`
    
    vault unseal `grep 'Key 1:' /root/vault_key.txt | awk '{print $NF}'`
    vault unseal `grep 'Key 2:' /root/vault_key.txt | awk '{print $NF}'`
